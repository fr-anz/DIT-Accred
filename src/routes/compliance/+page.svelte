<script>
	...
	// Typed so `noImplicitAny` does not flag the empty array literal.
	/** @type {HTMLDivElement[]} */
		...
		<div class="compliance-container">
			...
			<div class="timeline" bind:this={timelineContainer}>
				...
				{#each complianceAreas as area, i}
					...
					<div
						class="area-row {activeArea === area.id ? 'scroll-active' : ''}"
...
<style>
	...
	.hero-title {
		...
		margin: 0;
		background: linear-gradient(to right, #000000 0%, var(--color-maroon) 80%);
	...
	.ambient-scroll-light {
		...
		box-shadow:
			...
			0 0 45px 15px rgba(76, 175, 80, 0.5);
		z-index: 100;
	...
	.left-half {
		...
		padding-right: var(--card-distance);
	}
	.right-half {
		justify-content: flex-start;
		padding-left: var(--card-distance);
	}
